import { RateLimitInfo, RedisManager } from "../services/valkey";
...
export class RateLimiter {
  ...
  static async isRateLimited(ip: string, username?: string): Promise<boolean> {
    ...
    if (username) {
      ...
      const userInfo: RateLimitInfo | null =
        await RedisManager.getRateLimit(userKey);

      if (userInfo) {
        if (
          userInfo.blocked &&
          now - userInfo.lastAttempt < this.BLOCK_DURATION
        ) {
          return true;
        }
  ...
  static async trackAttempt(ip: string, username?: string): Promise<void> {
    ...
    if (username) {
      const userKey = this.getKey("user", username);
      const userInfo: RateLimitInfo = (await RedisManager.getRateLimit(
        userKey,
      )) || {
        attempts: 0,
        firstAttempt: now,
